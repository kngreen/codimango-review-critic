...
3. Create a distinct `review-trials-and-spec` child. For iOS, create a distinct `aai-ios` child and record native target/XCTest evidence. Every child must run `emit_run_receipt.py`; build its manifest record only with `agentcloud_adapter.py` from the durable session journal.
